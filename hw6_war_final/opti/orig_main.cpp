...
WDragon::WDragon(int armyname_, int num_total_warrior_, int M_):Warriors(0, armyname_, num_total_warrior_){
    ...
    force = g_attack[0];
    morale=((double)M_/(double)element);
    cout << "Its morale is ";
    //cout.precision(2);cout.setf(ios::fixed);
    //if ((M_*1000/element%10)==5)
            // morale-=0.01;//
           //  cout << M_<< "/" << element <<"="<< morale <<" => ";
...
Warriors* Headquarters::Produce() {
  ...
  if(( M<g_strength[ORDER[sequence_num][order_counter]] ) ) {  
    ...
    while((!gameover)&&(dead_loop--)){
        switch(minute){
		    ...
		    case(5): {//lions runaway
			          for (int i=0; i< n_total_city + 1; i++)
			              for (int j=0; j<1; j++)
		    ...
		    case(35): {//Arrow shooting
			        for (int i=1; i < n_total_city+1; i++){//red
   		                if( city_v[i]->warr[0]!=NULL )//has warroir with arror, and next city has enemy 
                            if( (city_v[i]->warr[0]->arsenal[2]!=NULL) && (city_v[i+1]->warr[1]!=NULL ))
						        if(i!=n_total_city&& city_v[i]->Shooting(city_v[i+1],0)==1 ){ city_v[i+1]->warr[1]->element=0; }
   		                if( city_v[i]->warr[1]!=NULL ) 
                            if( (city_v[i]->warr[1]->arsenal[2]!=NULL) && (city_v[i-1]->warr[0])!=NULL ) 
                              if( i!=1&&city_v[i]->Shooting(city_v[i-1],1)==1 ) city_v[i-1]->warr[0]->element=0;
                    }
 		      break;
		    }
		    case(38): {//Bombing
