...
//const string WEAPONSTR[3] = {"sword", "bomb", "arrow"};
...
class Headquarters {
  ...
  public:
    ...
    void Produce() {
      int j=0;
      string WEAPONSTR[3]= {"sword","bomb","arrow"};
      //cout << "weapon is "<< WEAPONSTR[1] << endl;
      ...
      if ( j==5 ) {
        ...
        switch( ORDER[sequence_num][order_counter]) {
          case(0): {float morale=(float)M/(float)each_warrior_strength[ORDER[sequence_num][order_counter]]; 
                     //cout.precision(3);
                     cout << "It has a " << WEAPONSTR[WEAPON[n_warriors%3]] << ",and it's morale is "  ;
                     printf("%4.2f",morale);
                     cout << endl;
                    break;} 
          case(1): cout << "It has a " <<  WEAPONSTR[WEAPON[n_warriors%3]] << " and a " << WEAPONSTR[WEAPON[(n_warriors+1)%3]] << endl;break;
          case(2): cout << "It has a " <<  WEAPONSTR[WEAPON[(n_warriors)%3]] << endl;break; 
          case(3): cout << "It's loyalty is " << M << endl; break;
          case(4): {break;}
          default: cout << "Error weapon" << endl;break;
        }
        order_counter++;
...
//Initiation
  ...
  for(round=0;round!=test_case;round++){
    ...
    //Read input data from previous procedures 
